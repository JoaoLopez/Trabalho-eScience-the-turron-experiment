...
def main():
    ...
    def read_csv_and_preprocess_data(filename):
        ...
        temp = pd.melt(df, id_vars=['name', 'gender', 'first_time_tasting', 'correct_guess', 'hours since last eat'], value_vars=[
        ...
        temp = melted[melted.variable == 'sweetness']
        melted = melted.drop(temp.index)
    ...
    temp1 = df['gender']
    temp2 = df['gender']
    sizes = [temp1.value_counts()['male'],
    temp2.value_counts()['female']]
    ...
    temp1 = df_naive['first_time_tasting']
    temp2 = df_naive['first_time_tasting']
    temp3 = df_naive['first_time_tasting']
    sizes = [temp1.value_counts()['Y'],
    temp2.value_counts()['N'],
    temp3.value_counts()['NA']
    ]
    ...
    def gender_by_turron_per_category(melted):
        for param  in ['texture', 'flavour', 'visual', 'sweetness', 'overall']:
            ...
            param_a = param+"A"
            param_b = param+"B"
            temp = melted['composite_variable']
            temp1 = '|'
            mask = temp.str.contains(temp1.join([param_a, param_b]))
    ...
    def gender_general_effect(melted):
        ...
        temp = g.axes[0,0]
        temp.set_ylabel('mean score')
        temp = g.axes[0,0]
        temp.xaxis.label.set_visible(False)
        temp = g.axes[0,0]
        temp.text(4, 1.5, f"p-values:", fontweight='bold')
        temp = g.axes[0,0]
        temp1 = '\n'
        temp.text(4, 0, temp1.join(p_values))
        return g
    ...
    for (cat_A, cat_B) in comparisons:
        for gender in ('male', 'female',):
            ...
            temp = df_cat['delta']
            print(f"P Value gender: {gender};\t{cat_A} vs {cat_B}\t= {pvalue:1.4f} / delta mean {temp.mean():1.4f}")
    ...
    def turron_by_first_time_tasting2(melted):
        ...
        temp.xaxis.label.set_visible(False)
        temp = g.axes[0,1]
        temp.set_xlabel('Was is the first time to taste turron?')
        temp = g.axes[0,2]
        temp.xaxis.label.set_visible(False)
        temp = g.axes[0,3]
        temp.xaxis.label.set_visible(False)
    ...
    #####PARTE DO PROCESSMANETO É DETERMINÍSTICO. ENTRETANTO POSSUI COMANDO PRINT
    def paired_t_test_turron_by_naiveness(df):
        ...
        for (cat_A, cat_B) in comparisons:
            for first_time_taster in ('N', 'Y',):
                df_cat = df[[cat_A, cat_B, 'first_time_tasting']]
                df_cat = df_cat.dropna()
                df_cat['delta'] = df_cat[cat_A] - df_cat[cat_B]
                ...
                statistic, pvalue = stats.ttest_rel(df_cat[cat_A], df_cat[cat_B])
                temp = df_cat['delta']
                print(f"P Value first_time_tasting: {first_time_taster};\t{cat_A} vs {cat_B}\t= {pvalue:1.4f} / delta mean {temp.mean():1.4f}")
    ...
    def turron_by_correct_guess2(melted):
        ...
        temp.set_ylabel('mean score')
        temp = g.axes[0,1]
        temp.set_xlabel('Guessed which turron was expensive')
        temp = g.axes[0,2]
        temp.xaxis.label.set_visible(False)
        temp = g.axes[0,3]
        temp.xaxis.label.set_visible(False)
    ...
    temp1 = df['guess_expensive']
    temp2 = df['guess_expensive']
    sizes = [temp1.value_counts()['A'],
    temp2.value_counts()['B']]
    ...
    def success_rate_by_naiveness(df):
        ...
        temp = contingency.loc['N']
        non_naive = temp.sum()
        temp = contingency.loc['Y']
        naive = temp.sum()
    ...
    def success_rate_by_gender(df):
        ...
        oddsratio, pvalue = stats.fisher_exact(contingency)
        print(f"p-value: ", pvalue)
        temp = contingency.loc['male']
        male = temp.sum()
        temp = contingency.loc['female']
        female = temp.sum()
    ...
    for (cat_A, cat_B) in comparissons:
        df_cat = df[[cat_A, cat_B]]
        df_cat = df_cat.dropna()
        df_cat['delta'] = df_cat[cat_A] - df_cat[cat_B]

        statistic, pvalue = stats.ttest_rel(df_cat[cat_A], df_cat[cat_B])
        temp = df_cat['delta']
        print(f"P Value {cat_A} vs {cat_B} = {pvalue} / delta mean {temp.mean()}")
    ...
    def influence_of_fasting(melted):
        temp = melted[['variable', 'turron', 'hours since last eat', 'value']]
        melted_hours = temp.dropna()
        correlations  = {}
        for variable in ['texture', 'visual', 'flavour', 'overall']:
    ...
    def influence_of_fasting_by_naiveness(melted):
        temp = melted[['variable', 'turron', 'hours since last eat', 'value', 'first_time_tasting']]
        melted_hours = temp.dropna()
        correlations  = {}
        for variable in ['texture', 'visual', 'flavour', 'overall']:
            correlations[variable] = {}
    ...
    def preference_coherence(df):
        ...
        temp = df[['overall_A', 'overall_B', 'preference', 'guess_expensive', 'coherence']]
        coherence_df = temp.dropna()
        coherence_df

        temp = coherence_df.groupby('coherence')
        coherence_count = temp.agg({'coherence': 'count'})
        ...
        temp = coherence_count['%'][0:2]
        plt.bar(temp.sum(), 1, width=coherence_count['%'][2], align='edge', color='lightgray'    , label='Tie')
    ...
    def influence_of_naiveness_on_score(df, with_individuals=True):
        ...
        temp = df['first_time_tasting']
        unknown = df[temp.isnull()]
    ...
    def format_for_sankeymatic(data):
        ...
        temp = '\n'
        return temp.join(list(map(format_row, data)))
